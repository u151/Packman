...
Ground::Ground(GameObject* parent)
    :GameObject(parent, "Ground"), hModel_(-1), mapWidth_(-1), mapHeight_(-1), normalEsaCount_(0), powerEsaCount_(0), respawnEsa_(false)
{
    CsvReader csvData;
...
{
    hModel_ = Model::Load("dodai5.fbx");
    hModelt_ = Model::Load("block3.fbx");
   /* hEsaModel_ = Model::Load("esa3.fbx");
    hPEsaModel_ = Model::Load("P_esa2.fbx");*/
    RespawnEsa();
}
...
void Ground::DecEsaCount(EsaType type)
{
    if (type == ESATYPE_NORMAL)
    {
        normalEsaCount_--;
    }
    else if (type == ESATYPE_POWER)
    {
        powerEsaCount_--;
    }

    if (normalEsaCount_ <= 0 && powerEsaCount_ <= 0)
    {
        respawnEsa_ = true;
    }
}

void Ground::RespawnEsa()
{
    normalEsaCount_ = 0;
    powerEsaCount_ = 0;
    ...
    for (int x = 0; x < mapWidth_; x++)
    {
        for (int y = 0; y < mapHeight_; y++)
        {
            if (objMap_[y][x] == 1)
            {
                Esa* esa = (Esa*)Instantiate<Esa>(this);

                esa->SetPosition({-9.0f + x * 2.0f, 0.0f, 9.0f - y * 2.0f});

                esa->SetEsaType(ESATYPE_NORMAL);
